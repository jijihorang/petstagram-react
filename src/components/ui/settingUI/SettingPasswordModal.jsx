...
const Button = styled.button`
  ...
  &:hover {
    background-color: ${(props) => (props.disabled ? '#e6e6fa' : '#dda0dd')};
  }
`;
...
const SettingPasswordModal = ({ isOpen, onClose, profileInfo }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // 비밀번호 변경
      await UserService.updatePassword(profileInfo.id, newPassword, token);
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <SmallText>비밀번호는 최소 6자 이상이어야 하며 숫자, 영문, 특수 문자(!$@%*)의 조합을 포함해야 합니다.</SmallText>
          ...
          <CheckboxContainer>
            ...
            <CheckboxLabel htmlFor="logout-other-devices">
              다른 기기에서 로그아웃합니다. 다른 사람이 회원님의 계정을 사용한 경우 선택하세요.
            </CheckboxLabel>
          </CheckboxContainer>
